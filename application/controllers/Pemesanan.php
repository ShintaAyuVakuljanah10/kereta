...
class Pemesanan extends CI_Controller {
	...
	public function add() {
		if ($this->input->post()) {
			...
			// Buat kode_tiket otomatis tanpa id_kereta
			$kode_tiket='TK-'. date('YmdHi') . '-'. $id_tiket . '-'. $id_gerbong;

			// Siapkan data pemesanan
			$data_pemesanan=[ 'id_tiket'=>$id_tiket,
	...
	public function bayar() {
		...
		$this->db->where('id_pemesanan', $id_pemesanan) ->update('pemesanan', [ 'status'=> 'lunas',
	...
	public function edit($id) {
		if($this->input->post()) {
			$data=[ "id_tiket"=>$this->input->post("id_kereta"),
